// quiz-logic.js - Integrasi lengkap dengan questions database
// File ini mengintegrasikan questions.js dengan quiz interface
...
class QuizManager {
  ...
  updateGlobalProgress() {
    ...
    const percentage = Math.min((answered / totalQuestions) * 100, 100); // Pastikan tidak lebih dari 100%
  ...
  getTotalAnswered() {
    ...
    // Hitung hanya level yang benar-benar completed
        ...
        Object.keys(progress[topic]).forEach((difficulty) => {
          if (
            ...
            Object.keys(progress[topic][difficulty]).forEach((level) => {
              ...
              // Pastikan level benar-benar completed dan memiliki data yang valid
              if (
                levelData &&
                ...
                total += levelData.answers.length; // Gunakan jumlah jawaban yang sebenarnya
              }
            });
          }
        });
  ...
  loadProgress() {
    ...
    // Reset progress bar jika data tidak valid
